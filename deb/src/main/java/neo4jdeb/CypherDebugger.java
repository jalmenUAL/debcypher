...
public class CypherDebugger {
    ...
    statement.accept(segment -> {
        if (segment instanceof Relationship rel) {
            // Extraer etiquetas de los nodos (origen y destino)
            String startLabel = extractLabel(rel.getLeft());
            String endLabel = extractLabel(rel.getRight());
            
            // Extraer el tipo de relación (el primero si hay varios)
            String relType = rel.getDetails().getTypes().isEmpty() ? null 
                             : rel.getDetails().getTypes().get(0);

            if (startLabel != null && endLabel != null && relType != null) {
                String pathKey = startLabel + "-" + relType + "-" + endLabel;
                if (!processedPaths.contains(pathKey)) {
                    runDeepConnectivityCheck(session, startLabel, relType, endLabel);
...
private void runDeepConnectivityCheck(Session session, String start, String type, String end) {
    ...
    String reverseQ = String.format("MATCH (a:%s)<-[:%s]-(b:%s) RETURN count(*) > 0 AS ok LIMIT 1", end, type, start);
    ...
    if (res.hasNext()) {
        ...
        System.err.println(String.format("      Nodes are not linked by [:%s], but they share a (:%s) node.", type, mid));
        System.err.println(String.format("      👉 SUGGESTED MATCH: (:%s)-[:%s]-(:%s)-[:%s]-(:%s)", start, t1, mid, t2, end));
    } else {
    ...
    for (String rawCondition : conditions) {
        ...
        if (rawCondition.contains("cypher=")) {
            cleanCondition = rawCondition.substring(rawCondition.indexOf("cypher=") + 7, rawCondition.lastIndexOf("}"));
        }
...
private void checkPropertyTypeMismatch(Session session, String propertyName, String condition) {
    ...
    public static void main(String[] args) {
        ...
        // Example with a logical condition that might fail (e.g., movies from the future)
        String testQuery = "MATCH (:Person {name:'Tom Hanks'})-[r:!ACTED_IN]->(m:Movie)\n" + //
